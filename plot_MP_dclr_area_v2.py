...
mps = ['Tho','GCE','WSM','Mor']
subgs = [''] # 'T' or ''
subgt = ['-SM2-'] # 'TKE' or 'SM2'
for i,subg in enumerate(subgs):
    for mp in mps:
        if mp == 'Tho' and subg == '':
            pbls = ['YSU','YDE', 'BL', 'BHE']
            df = pd.read_csv(mdir+'Data_to_plot/Size_and_number_coresMPs.csv')
        else:
            pbls = ['YSU','BL']
        for pbl in pbls:
...
for i,run in enumerate(runs):
    ...
    #### CP intensity
    cps.append(dfcp[run][48:dia*24].mean())
    #### W
...
for i,run in enumerate(runs):
    plt.scatter(dist[i], cps[i]*3.6, label = titles[i], color = color[i], marker = marker[i])
...
for i,run in enumerate(runs):
    plt.scatter(numm[i], cps[i]*3.6, label = titles[i], color = color[i], marker = marker[i])
...
for i,run in enumerate(runs):
    plt.scatter(dist[i], ww_cp[i], label = titles[i], color = color[i], marker = marker[i])
...
for i,run in enumerate(runs):
    plt.scatter(rh[i], cps[i]*3.6, label = titles[i], color = color[i], marker = marker[i])
...
for i,run in enumerate(runs):
    ...
    plt.plot(bin_centers*bin_centers*np.pi, y, label = titles[i], color = color[i], linestyle = linestyles[i])
...
plt.savefig(scra+'MP_dclr_'+subgs[0]+'_area_v2.jpg', bbox_inches = 'tight', dpi = 300)
plt.savefig(scra+'MP_dclr_'+subgs[0]+'_area_v2.pdf', bbox_inches = 'tight', dpi = 300)
plt.show()
